...
#SBATCH -t 0-3:00:00
#SBATCH -o /data/users_area/yky10kg/GREENrice/Cons_Gen/datasets/herbarium/trial/log/3_QualityCheck_%A_%a.log
#SBATCH -e /data/users_area/yky10kg/GREENrice/Cons_Gen/datasets/herbarium/trial/log/3_QualityCheck_%A_%a.err
...
${BIN}/samtools flagstat ${HOM}/mapped/${SAMPLE}.mapped.sort.rmdup.bam >> ${HOM}/QC/quality_check_${SAMPLE}.txt

#Unmapped reads
echo -e "\nUnmapped reads:" >> ${HOM}/QC/quality_check_${SAMPLE}.txt
${BIN}/samtools flagstat ${HOM}/unmapped/${SAMPLE}.unmapped.sort.rmdup.bam >> ${HOM}/QC/quality_check_${SAMPLE}.txt

#Read stats
echo -e "\nRead stats:" >> ${HOM}/QC/quality_check_${SAMPLE}.txt
echo "SAMPLE=${HOM}/mapped/${SAMPLE}.mapped.sort.rmdup.bam" >> ${HOM}/QC/quality_check_${SAMPLE}.txt
${BIN}/samtools stats ${HOM}/mapped/${SAMPLE}.mapped.sort.rmdup.bam | grep ^SN | cut -f 2- >> ${HOM}/QC/quality_check_${SAMPLE}.txt
